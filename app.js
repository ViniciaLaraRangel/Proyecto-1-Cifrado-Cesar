...
//Conseguir la palabra desde HTML para declararla y convertirla a Mayúsculas
    ...
    //console.log(deCodex);
//Paso 1: MODIFICAR Condicionar el bucle para recorrer la posición de la palabra introducida==> Tip: charCodeAt
for (let posicionB =0; posicionB<deCodex.length; posicionB ++){
    //console.log(posicionB);
//Paso 2: MODIFICAR Obtener de acuerdo a la posiciónA, la nueva posición Ascii
let palabraCodex = deCodex.charCodeAt(posicionB);
    //console.log(palabraCodex);
/*Paso 3: MODIFICAR Aplicar la fórmula ((((x-65) + n) % 26) + 65) en donde:*/
/*x es la posición ASCII y n el "key" para codificar*/
let desenCodex = (palabraCodex-65-key) % 26 + 65;
    if (desenCodex <65) {
        desenCodex = desenCodex + 26;
        console.log(desenCodex);
    } 

//Paso 4: MODIFICAR Con la posición ASCII obtenida, devolver cadena obtenida ==> Tip: fromCharCode
let deCodexResultado = String.fromCharCode(desenCodex);
//console.log(deCodexResultado);
//Paso 5: MODIFICAR Imprime resultado en input de codificado, fórmula resumida
